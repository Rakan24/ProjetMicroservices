...
<head>
    ...
    <style>
        ...
        .hero {
            ...
            color: white;
            height: 500px;
            background: url("images/img/pimg.jpg") no-repeat center center / cover;
            display: flex;
            align-items: center;
        ...
        <nav class="navbar navbar-dark">
            <div class="container">
                ...
                <a class="navbar-brand" href="index.php">
                    <img class="img-rounded" src="images/logo.png" alt="Logo" width="18%">
                </a>
            ...
            <div class="steps">
                <div class="step-item step1">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 483 483" width="80" height="80">
                        <path d="..."></path> <!-- Simplified for brevity -->
                    </svg>
...
<section class="popular">
    <div class="container">
        ...
        <?php
$query_res = $pdo->query("SELECT * FROM plats LIMIT 6");
while ($r = $query_res->fetch(PDO::FETCH_ASSOC)) {
    echo '
    <div class="col-xs-12 col-sm-6 col-md-4 food-item">
        <div class="food-item-wrap">

            <div class="figure-wrap bg-image" style="background-image: url(\'Utilisateur/Res_img/dishes/' . htmlspecialchars($r['image_url']) . '\');"></div>

            
            <div class="content">

                <h5><a href="dishes.php?res_id=' . htmlspecialchars($r['restaurant_id']) . '">' . htmlspecialchars($r['nom']) . '</a></h5>
                <div class="product-name">' . htmlspecialchars($r['description']) . '</div>
                <div class="price-btn-block">
                    <span class="price">$' . htmlspecialchars($r['prix']) . '</span>
                    <a href="dishes.php?res_id=' . htmlspecialchars($r['restaurant_id']) . '" class="btn theme-btn-dash pull-right">Order Now</a>
                </div>
            </div>
        </div>
    </div>';
                ...
                <div class="restaurants-filter pull-right">
                    <nav class="primary pull-left">
                        <ul>
                            ...
                            <?php 
$query = $pdo->query("SELECT * FROM res_category");
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    echo '<li><a href="#" data-filter=".' . htmlspecialchars($row['c_name']) . '"> ' . htmlspecialchars($row['c_name']) . '</a></li>';
}
?>
        ...
        <div class="row">
            <div class="restaurant-listing">
                ...
                $query_res = $pdo->query("SELECT r.image, r.nom, r.adresse, c.c_name, r.id FROM restaurant r JOIN res_category c ON r.c_id = c.c_id");
                while ($r = $query_res->fetch(PDO::FETCH_ASSOC)) {
                    echo '
                    <div class="col-xs-12 col-sm-12 col-md-6 single-restaurant all ' . htmlspecialchars($r['c_name']) . '">
                        <div class="restaurant-wrap">
                            <div class="row">
                                <div class="col-xs-12 col-sm-3 col-md-12 col-lg-3 text-xs-center">
                                    <a class="restaurant-logo" href="dishes.php?res_id=' . htmlspecialchars($r['id']) . '"> 
                                    <img src="Utilisateur/Res_img/' . htmlspecialchars($r['image']) . '" alt="Restaurant logo">

                                    </a>
                                </div>
                                <div class="col-xs-12 col-sm-9 col-md-12 col-lg-9">
                                    <h5><a href="dishes.php?res_id=' . htmlspecialchars($r['id']) . '">' . htmlspecialchars($r['nom']) . '</a></h5> 
                                    <span>' . htmlspecialchars($r['adresse']) . '</span>
                                </div>
                            </div>
                        </div>
                    </div>';
